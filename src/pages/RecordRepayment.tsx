...
import {
  usePeopleForRepayment,
  useActiveLoansForPerson,
  useCreateRepayment,
...
export default function RecordRepayment() {
  ...
  const repaymentType: RepaymentType =
    type === "pay" || type === "receive" ? type : "pay";
  const { appUser } = useAuth();
  const { toast } = useToast();
  const { data: peopleForRepayment = [] } =
    usePeopleForRepayment(repaymentType);
  ...
  const { data: userLoans = [] } = useActiveLoansForPerson(
    selectedPerson,
    repaymentType,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedLoan === "__manual__") {
      toast({
        title: "Алдаа",
        description: "Зээлээ сонгоно уу.",
        variant: "destructive",
      });
      return;
    }

    // pay: төлсөн хүн = appUser (borrower). receive: төлсөн хүн = selectedPerson (borrower)
    const personName =
      repaymentType === "pay"
        ? appUser?.nickname ||
          appUser?.username ||
          appUser?.userCode ||
          "Хэрэглэгч"
        : (personOptions.find((p) => p.id === selectedPerson)?.name ?? "");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <form
        ...
        className="p-6 space-y-6 md:max-w-xl md:mx-auto md:px-8"
      >
        <div className="bg-card rounded-2xl border border-border p-5 shadow-sm space-y-5">
          <div className="space-y-2.5">
            <label className="text-xs text-muted-foreground font-medium">
              {repaymentType === "pay"
                ? "Зээл өгсөн хүн (төлбөр төлөх хүн)"
                : "Зээл авсан хүн (төлбөр авах хүн)"}
            </label>
            <ContactSelect
          ...
          {selectedPerson && (
            <div className="space-y-2.5">
              <label className="text-xs text-muted-foreground font-medium">
                {repaymentType === "pay"
                  ? "Өгөх зээлийн дүн сонгоно уу"
                  : "Авах зээлийн дүн сонгоно уу"}
              </label>
              <LoanSelect
                loans={userLoans}
                value={selectedLoan}
          ...
          <div className="space-y-2.5">
            ...
            >
              ...
              <ChevronDown
                className={`h-4 w-4 transition-transform ${showBankInfo ? "rotate-180" : ""}`}
              />
            </button>
            {showBankInfo && (
              <div className="space-y-4 pt-2">
                <div className="space-y-2.5">
                  ...
                  <BankAccountInput
                    bank={senderBank}
                    onBankChange={setSenderBank}
                    account={senderAccount}
                    onAccountChange={setSenderAccount}
                  />
                </div>
                <div className="space-y-2.5">
                  <label className="text-xs text-muted-foreground font-medium">
